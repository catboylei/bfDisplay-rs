...
// function called by backend when ping is received
...
export function setupSyntax(): void {
    ...
    vim.api.nvim_command(op ? `hi bfOperator guifg=${op}` : 'hi def link bfOperator Operator'); // if config is null or default, use user theme
